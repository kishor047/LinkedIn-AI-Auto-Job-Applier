...
def get_installed_chrome_info() -> tuple[str | None, int | None]:
    """Return the installed Chrome executable path and major version if available."""
        ...
        for chrome_path in candidate_paths:
            if os.path.exists(chrome_path):
                try:
                    ...
                    output = str(e.output or '').strip()
                match = re.search(r"(\d+)\.", output)
                if match:
                    return chrome_path, int(match.group(1))
...
def createChromeSession(isRetry: bool = False):
    ...
    chrome_executable_path, chrome_version_main = get_installed_chrome_info()
    if stealth_mode:
        if chrome_executable_path:
            print_lg(f"Using installed Chrome at: {chrome_executable_path}")
        if chrome_version_main:
            print_lg(f"Detected Chrome major version: {chrome_version_main}")

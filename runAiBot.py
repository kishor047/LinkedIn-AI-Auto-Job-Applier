...
def external_apply(pagination_element: WebElement, job_id: str, job_link: str, resume: str, date_listed, application_link: str, screenshot_name: str) -> tuple[bool, str, int]:
    ...
    try:
        ...
        if close_tabs and driver.current_window_handle != linkedIn_tab:
            if application_link and 'linkedin.com' not in application_link:
                driver.close()
            else:
                print_lg('External apply tab opened, but it does not appear to be an external application page yet. Keeping the tab open for review.')
    ...
    for searchTerm in search_terms:
        ...
        try:
            while current_count < switch_number:
                ...
                if not job_listings:
                    print_lg(f'No jobs found for "{searchTerm}" on this page. Trying the next role...')
                    break
